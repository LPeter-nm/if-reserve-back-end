...
export class UserExternalService {
  ...
  async register(body: CreateUserExternalDto) {
    ...
    const usrExternalCheck = await this.prisma.user_External.findFirst({
    ...
    const cpfCheck = await this.prisma.user_External.findUnique({
      where: { cpf: body.cpf },
    });
    if (cpfCheck)
  ...
  async update(id: string, body: CreateUserExternalDto) {
